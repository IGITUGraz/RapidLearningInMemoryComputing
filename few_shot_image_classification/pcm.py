...
from quantize import no_quantization
...
class PCM:
    def __init__(self, quantization_func=None, nb=8, a=1.):
        self.nb = nb
        self.a = a
        if quantization_func is None:
            self.quantization_func = no_quantization
        else:
            self.quantization_func = quantization_func

    def init_weights(self, w):
        return self.quantization_func(w, self.nb, self.a)

    def program(self, w):
        return self.quantization_func(w, self.nb, self.a)

    def re_program(self, w, dw):
        return self.quantization_func(w + dw, self.nb, self.a)
...
class PCMNoise:
    def __init__(self, quantize_func, w_max=1.0, g_max=25.0, t_start=32, nb=None, a=None):
    ...
    def read_noise(w):
        ...
        sigma = tf.math.abs(g_t) * q_s * tf.math.sqrt(tf.math.log((t_start + t_read) / (2*t_read)))
    ...
    def read_noise_sigma(w):
        ...
        return sigma * tf.sqrt(number_of_synapses * ((1 / number_of_synapses) ** 2)) / g_max
    ...
    def re_program(self, w, delta_w):
        delta_w = self.quantize_func(delta_w)
